...
Measure::Measure(const float *buffer, size_t length):
...
void Measure::setDataSource(const float *buffer, size_t length)
{
	m_buffer = buffer;
	m_buf_length = length;
}

TimeMeasure::TimeMeasure(const float *buffer, size_t length) : Measure(buffer, length) {
...
bool TimeMeasure::highLowFromHistogram(double &low, double &high,
...
SpectralMeasure::SpectralMeasure(const float *buffer, size_t length) : Measure(buffer, length) {
...
std::shared_ptr<MeasurementData> Measure::measurement(QString str) {
